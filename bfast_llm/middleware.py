...
class BFastLLM:
    ...
    def compress_messages(self, messages: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], bool]:
        ...
        for msg in messages:
            ...
            if content is not None and isinstance(content, str) and len(content) >= self.threshold_bytes:
                ...
                if parsed_data is not None:
                    ...
                    try:
                        binary_data = self.encoder.encode_packed(parsed_data, compress=self.compress_payloads)
                        ...
                        print(f"[BFastLLM] Warning: Failed to serialize payload: {e}", file=sys.stderr)
            ...
            # Also handle if the content is a dictionary, list, NumPy array, or Pandas DataFrame
                ...
                if is_supported:
                    try:
                        ...
                        binary_data = self.encoder.encode_packed(processed_content, compress=self.compress_payloads)
                        ref_tag = self.registry.register(binary_data)
                        new_msg = msg.copy()
                        new_msg["content"] = (
                            f"The data was compressed using B-FAST to save tokens:\n{ref_tag}\n"
                        ...
                        print(f"[BFastLLM] Warning: Failed to serialize object/array payload: {e}", file=sys.stderr)
    ...
    def handle_tool_call(self, tool_call: Any) -> Optional[str]:
        ...
        name = getattr(tool_call, "name", None) or getattr(getattr(tool_call, "function", None), "name", None)
        arguments = getattr(tool_call, "arguments", None) or getattr(getattr(tool_call, "function", None), "arguments", None)
...
def wrap_completion(completion_func, threshold_bytes: int = 1024, compress_payloads: bool = True):
    """
    ...
    """
    bfast_llm = BFastLLM(threshold_bytes=threshold_bytes, compress_payloads=compress_payloads)
    ...
    def wrapped(*args, **kwargs):
        ...
        while True:
            ...
            choices = response.get("choices", []) if is_dict else getattr(response, "choices", [])
            if not choices:
                return response
                
            message = choices[0].get("message", {}) if is_dict else choices[0].message
            tool_calls = message.get("tool_calls", []) if is_dict else getattr(message, "tool_calls", None)
            ...
            for tc in tool_calls:
                name = tc.get("function", {}).get("name") if is_dict else getattr(getattr(tc, "function", None), "name", None)
                if name == "bfast_retrieve":
...
def bfast_tune(client, threshold_bytes: int = 1024, compress_payloads: bool = True):
    ...
    wrapped_create = wrap_completion(original_create, threshold_bytes, compress_payloads)
